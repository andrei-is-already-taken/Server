...
(function() {
  var Avatar, Category, Comment, CommentRating, Creative, CreativeRating, DestroyTags, Medal, Model, Passport, Promise, Rating, Sequelize, Tag, User, UserRating, async, cloudinary, conf, sequelize, util;
  ...
  DestroyTags = function(creative) {
    var allTags, creativeTags;
    creativeTags = creative.getTags();
    allTags = Model.Tag.findAll();
    return Promise.all([creativeTags, allTags]).spread(function(creativeTags, allTags) {
      var allIds, creativeIds, diff, toDelete;
      creativeIds = creativeTags.map(function(tag) {
        return tag.id;
      });
      allIds = allTags.map(function(tag) {
        return tag.id;
      });
  ...
  CreativeRating = sequelize.define('CreativeRating', {
    score: Sequelize.INTEGER
  });

  UserRating = sequelize.define('user_rating', {
    score: Sequelize.INTEGER
  });
  ...
  Creative.belongsToMany(Tag, {
    through: 'CreativeTag'
  });

  Tag.belongsToMany(Creative, {
    through: 'CreativeTag'
  });
  ...
  Model = {
    ...
    AddScores: function(creatives, ratings) {
      var i, sums;
      sums = ratings.map(function(ratings) {
        var sum;
        sum = 0;
    ...
    AddRatables: function(creatives, user) {
      ...
      allRatings = Promise.all(creatives.map(function(creative) {
      ...
      return allRatings.then(function(allRatings) {
        ...
        while (i < allRatings.length) {
          alreadyRated = allRatings[i].some(function(rating) {
    ...
    AddUsers: function(creatives) {
      var userPromises;
      userPromises = creatives.map(function(creative) {
    ...
    AddTags: function(creatives) {
      var tagsPromises;
      tagsPromises = creatives.map(function(creative) {
        return creative.getTags();
      });
      return new Promise(function(resolve, reject) {
        return Promise.all(tagsPromises).then(function(tags) {
          var i, tagValues;
          i = 0;
          while (i < creatives.length) {
            tagValues = tags[i].map(function(tag) {
