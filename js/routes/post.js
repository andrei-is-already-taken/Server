...
(function() {
  var Model, Promise, cloudinary, fs;
  ...
  exports.post = function(req, res) {
    var creative, tag_array, user;
    creative = cloudinary.uploadBase64(req.body.img, req.session.user).then(function(upload) {
      var map;
      map = void 0;
      if (req.body.map) {
    ...
    tag_array = Promise.all(req.body.tags.map(function(tag) {
    ...
    Promise.all([user, creative, tag_array]).spread(function(user, creative, tag_array) {
      var tags;
      tags = tag_array.map(function(tag_entry) {
        return tag_entry[0];
      });
      return [creative.addTags(tags), user.addCreative(creative)];
  ...
  exports.allForUser = function(req, res) {
    Model.User.findById(req.params.id).then(function(user) {
      ...
      return [
        creatives, Promise.all(creatives.map(function(creative) {
          return creative.getCreativeRatings();
        }))
      ];
    }).spread(Model.AddScores).then(Model.AddTags).then((function(creatives) {
      res.send(creatives);
    }), function() {
      res.sendStatus(403);
    });
  ...
  exports.getSpecificPost = function(req, res) {
    ...
    Model.Creative.findById(creativeId).then(function(post) {
      return [post];
  ...
  exports.getAllTags = function(req, res) {
    Model.Tag.findAll().then(function(tags) {
      ...
      counts = Promise.all(tags.map(function(tag) {
